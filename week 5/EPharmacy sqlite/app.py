...
app = Flask("EPharmacy")
app.config["SQLALCHEMY_DATABASE_URI"] = 'sqlite:///epharmacy.db'
db = SQLAlchemy(app)
...
def shop():
    ...
    if "google_id" in session:
        return render_template('shop.html', name = session["name"], picture = session["picture"], products=products, images=images)
    else:
        session.clear()
...
def view_product(product_id):
    ...
    if "google_id" in session:
        return render_template('product_details.html', name = session["name"], picture = session["picture"], product=product_detail)
    else:
...
def search_products():
    ...
    products = Product_info.query.filter(
        or_(
            Product_info.Manufacturer.like(f"%{search_query}%"),
            Product_info.BrandName.like(f"%{search_query}%"),
            Product_info.GenericName.like(f"%{search_query}%"),
            Product_info.Strength.like(f"%{search_query}%"),
            Product_info.Description.like(f"%{search_query}%"),
            Product_info.Price.like(f"%{search_query}%")
        )
    ).all()

    if "google_id" in session:
        return render_template('shop.html', name = session["name"], products=products, picture = session["picture"])
    else:
        session.clear()
...
def cart():
    if "google_id" in session:
        ...
        return render_template('cart.html', name = session["name"], picture = session["picture"], cart_items=cart_items, total_price=formatted_price)
    else:
...
def checkout():
    if "google_id" in session:
        ...
        return render_template('checkout.html', email = session["email"], name = session["name"], picture = session["picture"], cart_items=cart_items, total_price=formatted_price, address = user_address, phone = user_phone)
    else:
        return redirect(url_for('login'))
...
def history():
    if "google_id" in session:
        ...
        return render_template('history.html', name = session["name"], picture = session["picture"], history_items=history_items, total_price=formatted_price)
    else:
...
@login_is_required
def protected_area():
    return render_template('home.html', name = session["name"], picture = session["picture"]) 
